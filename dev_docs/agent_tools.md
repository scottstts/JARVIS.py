...
Running jobs do not emit model-facing heartbeats merely because output started or grew. Those observations update durable supervisor state and keep terminal detection alive without reviving the owner model. A no-output/stalled job emits at most one needs-attention wake after five minutes; terminal transitions always wake the owner. For child-owned jobs, the attention wake first resumes that child and does not independently escalate Jarvis; normal child failure, pause, approval, or blocker reporting remains the escalation path. Terminal notices include process exit/signal/runtime, command and output hashes, launch revision, bounded output tails, and durable stdout/stderr log paths. Exit status is process evidence, not semantic readiness evidence; record the actual acceptance check before reporting completion.
...
`acceptance_record` records the durable issue/acceptance ledger with statuses such as `open`, `fixed`, `not_a_bug`, `deferred`, and `user_waived`; it requires the exact `revision_paths` and still-current scoped revision from `acceptance_run`. Acceptance is a strict gate only for a subagent handing mutated work back to Jarvis. The harness, not the model, associates the ledger with the successful run that has the same revision scope and binds each contract requirement to one distinct compatible recorded check, so callers use criterion text rather than opaque tool-call or contract-item IDs. Legacy runs without per-scope gate evidence fail closed. Items still accumulate by stable stored IDs, so a later partial ledger cannot hide an older required open item. A subagent ledger reports `complete: false` together with concrete blockers whenever that handoff cannot yet pass; a main agent may use these tools voluntarily and is never completion-blocked by them. User corrections such as “no need” remove only the targeted or latest prior requirement. A passing process exit alone is not sufficient evidence of an externally observable requirement.
