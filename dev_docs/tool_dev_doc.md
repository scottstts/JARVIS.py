...
- the main route now receives queued agent-only subagent progress notices for `subagent_waiting_background`, `subagent_needs_attention`, and terminal child states through the same persisted-system-note plus runtime-turn pattern; the intermediate child-only `subagent_resumed_after_bash_update` notice no longer revives Jarvis by itself because it tended to create duplicate “still running” updates
