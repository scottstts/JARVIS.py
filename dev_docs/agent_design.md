...
At task creation Jarvis deterministically extracts explicit user requirements into a prompt-visible, persisted contract with stable item IDs. Child contracts are derived from the actual assignment instructions, user constraints, and deliverable rather than generic kickoff prose. A content-addressed contract revision is injected once per session, re-injected after compaction creates a new session, and injected again only when merged requirements materially change it; ordinary continuations reuse the persisted record instead of duplicating prompt bytes. Clear withdrawals including “no need” remove the targeted requirement, or only the latest requirement when no target is named. The contract is enforced as acceptance evidence only at a subagent handoff: verification-gate, authored-source-line, and visual-inspection requirements need matching runtime-observed evidence before a child mutation is accepted. The harness binds each requirement to one distinct compatible recorded check and a revision-matched run instead of requiring a child to echo opaque tool-call or contract-item IDs; legacy scopes without scoped gate evidence fail closed and require a fresh run. Test changes made by a child are still reported to the owning main task for visibility, but Jarvis itself is never completion-blocked by acceptance or contract matching. Slice rollover notices include the current progress epoch and outstanding contract items.
...
An unverified subagent-mutation handoff is an automatic continuation, not a terminal block. The attempted child response remains persisted for truthful replay, a concise acceptance instruction is appended, and the child loop continues with tools until current gates and the acceptance ledger resolve. Premature child completion text is buffered from the handoff while evidence is outstanding. A turn truthfully waiting on orchestrator-owned bash or subagent work remains non-blocked, records `task_completion_deferred`, retains the active task contract and acceptance state, and defers the acceptance continuation until pending work becomes terminal. The main agent is never put into this continuation by acceptance state. Only external/runtime blocks and the high unchanged-round liveness ceiling set `completion_blocked`, always with a structured `completion_block_reason`.
