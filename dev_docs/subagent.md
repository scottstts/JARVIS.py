...
Tool activity is rendered as one editable Telegram message per agent. Tool names are bold, repeated uses increment a count, and the most recently used tool is moved to the bottom of that agent's list. Every user-facing Jarvis message and gear-prefixed system notice partitions all current tool-activity stacks, so the next main-agent or subagent tool use starts a fresh notice below that message.
