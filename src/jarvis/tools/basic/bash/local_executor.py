...
def format_bash_tool_description(settings: ToolSettings) -> str:
    ...
    return (
        ...
        "`parallel`; `node`, `npm`, `npx`, and `corepack` are also available. "
        f"Python: use the central `{venv_root}` environment; use bare `python`/`python3` or "
        f"`{interpreter_path}`. Install Python packages with "
        f"`uv pip install --python {interpreter_path} <package-name>`. "
        f"Long-running jobs: foreground commands that are still running after the {settings.bash_foreground_soft_timeout_seconds:.0f}s "
